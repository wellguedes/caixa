...
$nf('#banco_nome').autocomplete(
	...
	$("#formulario").submit(function() {
		...
		$.post('envia.php', {nome: nome, valor: valor, caixa_id:caixa_id, cat:cat, data:data, tipo:tipo, pagamento:pagamento, banco_nome:banco_nome, numero_cheque:numero_cheque,servico:servico,referente:referente,convenio:convenio}, function(resposta) {
                ...
                $nf("#painel").load('index.php #painel').fadeIn("slow");
                $nf("#diario").load('index.php #diario').fadeIn("slow");
                $nf("#caixa").load('index.php #caixa').fadeIn("slow");
                $nf("#valores").load('index.php #valores').fadeIn("slow");
    ...
    <div id="conteudo-box">
      <div id="conteudo-box-in"> <a name="skip-menu"></a>
        ...
        <div class="div_centro">
          <h1 style="color: #000080; font-size: 35px; font-family: arial;">Caixa Clínica</h1>
          ...
          <form id="formulario" action="javascript:func()" method="post">
            <fieldset style="padding:20px; border:1px solid #e1e1e1; margin:0px 0px 10px 0px">
              ...
              <input type="hidden" name="caixa_id" id="caixa_id" value="1"/>
              <br/>
              <p>
                <label>Tipo:</label>
                <label>
                  <input type="radio" name="tipo" id="tipo" value="1" onclick="mostra('categoria')" />
                  Receita </label>
                <label>
                  <input type="radio" name="tipo" id="tipo" value="0" onclick="oculta('categoria')" />
                  Despesa </label>
              </p>
              <p>
                 <div id ="categoria" style="display:none; margin-bottom:10px">
                <label> Particular: </label>
                <select name="convenio" id="convenio" class="select" style="padding:3px; margin-bottom:10px;border-style:solid;border-width:2;border-color:#7AC6E7; ">
                <option selected value="">Selecione</option>
                <option value="Particular 15">Particular 15</option>
                <option value="Particular 20">Particular 20</option>
                <option value="Particular 28">Particular 28</option>
                <option value="Particular 32">Particular 32</option>
                </select>
                  </div>
              </p>
          ...
          <form id="fechar" action="javascript:func()" method="post">
            <?php
            $entrada = $functions->getEntradaClinica();
            ...
            $saidaDia = $functions->getSaidaClinica();
        ...
        <div class="conteudo-box-left">
          ...
          <?php
                        ...
                        $saida = $functions->getSaidaClinica();
...
#nome{
    ...
    #cross{display:none}
            ...
            $saidaDia = $functions->getSaidaDiaClinica();
            while($valor = mysql_fetch_array($saidaDia)) {
                // echo "<pre>";print_r($valor);echo "</pre>";
                $saiuDia = $valor['total'];
            }
