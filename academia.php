...
$("#flex1").flexigrid
        ...
        colModel : [
                ...
                {display: 'Referente à', name : 'referente', width : 125, sortable : true, align: 'left'},
                {display: 'Mês', name : 'data', width : 125, sortable : true, align: 'center'},
	...
	$("#formulario").submit(function() {
		...
		$.post('envia.php', {nome: nome, valor: valor, caixa_id:caixa_id, cat:cat, data:data, tipo:tipo, pagamento:pagamento, banco_nome:banco_nome, numero_cheque:numero_cheque,servico:servico,referente:referente}, function(resposta) {
                ...
                $nf("#painel").load('academia.php #painel').fadeIn("slow");
                $nf("#diario").load('academia.php #diario').fadeIn("slow");
                $nf("#caixa").load('academia.php #caixa').fadeIn("slow");
                $nf("#valores").load('academia.php #valores').fadeIn("slow");
    ...
    <div id="conteudo-box">
      <div id="conteudo-box-in"> <a name="skip-menu"></a>
        ...
        <div class="div_centro">
          <h1 style="color: #8B8378; font-size: 35px; font-family: arial;">Caixa Academia</h1>
          ...
          <form id="formulario" action="javascript:func()" method="post">
            <fieldset style="padding:20px; border:1px solid #e1e1e1; margin:0px 0px 10px 0px">
              ...
              <input type="hidden" name="caixa_id" id="caixa_id" value="4"/>
              <input type="hidden" name="link" id="link" value="academia.php"/>
              <br/>
              <p>
                <label>Tipo:</label>
                <label>
          ...
          <form id="fechar" action="javascript:func()" method="post">
            <?php
            $entrada = $functions->getEntradaAcademia();
            ...
            $saidaDia = $functions->getSaidaAcademia();
        ...
        <div class="conteudo-box-left">
          ...
          <?php
                        ...
                        $saida = $functions->getSaidaAcademia();
...
#nome{
    ...
    #cross{display:none}
              ...
              <?php
            $entrada = $functions->getEntradaDiaAcademia();
            while($valorDia = mysql_fetch_array($entrada)) {
                 //echo "<pre>";print_r($valorDia);echo "</pre>";
            ...
            $saidaDia = $functions->getSaidaDiaAcademia();
            while($valor = mysql_fetch_array($saidaDia)) {
                // echo "<pre>";print_r($valor);echo "</pre>";
                $saiuDia = $valor['total'];
            }
            ...
            $emDinheiro = $functions->getDinheiroAcademia();
            while($valor = mysql_fetch_array($emDinheiro)) {
                // echo "<pre>";print_r($valor);echo "</pre>";
                $entrouDinheiro = $valor['total'];
            }

            $emCheque = $functions->getChequeAcademia();
